...
# Run in a sandboxed environment (no DISPLAY, no GUI)
bwrap --ro-bind / / --dev /dev --tmpfs /tmp --chdir "$(pwd)" \
      env DISPLAY= TERM=dumb ./FreeFileSync_%{version}_Install.run --noexec --target extracted
