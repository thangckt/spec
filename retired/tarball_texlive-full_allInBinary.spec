...
### This spec is an older version of `tarball_texlive-full.spec` that installs the entire TeXLive distribution during the build stage. The newer version has been streamlined to install TeXLive during the post-installation stage, resulting in a much smaller package size for users to download and install.
